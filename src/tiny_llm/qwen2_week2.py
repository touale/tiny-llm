...
from tiny_llm.quantize import dequantize_linear, QuantizedWeights
...
class Qwen2MultiHeadAttention:
    ...
    def __call__(
        ...
        xq = rope(xq, offset=slice(offsets, offsets + x.shape[1]))
        xk = rope(xk, offset=slice(offsets, offsets + x.shape[1]))

        xq = xq.transpose(0, 2, 1, 3)
        xk = xk.transpose(0, 2, 1, 3)
        xv = xv.transpose(0, 2, 1, 3)

        (xk, xv) = cache.update_and_fetch(xk, xv)
...
class Qwen2TransformerBlock:
    ...
    def __call__(
        ...
        o1 = self.attention(o1, offsets=offset, mask=mask, cache=cache)
        o1 = x + o1
    ...
    def __call__(
        ...
        for i, layer in enumerate(self.layers):
            out = layer(
                ...
                cache=cache[i],
                offset=offset,
                mask="causal" if out.shape[1] > 1 else None,
            )
        out = self.norm(out)
